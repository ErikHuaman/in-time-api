...
export class AsistenciaController {
  constructor(
    ...
    private readonly asignacionService: AsignacionSedeUsuarioService,
  ...
  async findAllByMonth(
    ...
    const trabajadores = await this.trabajadorService.findAllByMonth(
      dto.fecha,
      user?.get()?.id,
    );
    let asistido = 0;
    const listAsistencia = trabajadores
      .map((t: Trabajador) => t.get())
      .map((t: Trabajador) => {
        ...
        const marcaciones = diasMes.map((d) => {
          ...
          const correccion = (asist?.correcciones ?? [])[0]?.get();
  ...
  async findAllBySupervisorAndDate(
    ...
    const asignaciones = await this.asignacionService.findByIdUsuarioAndFecha(
      diaMes.fecha,
      user?.get()?.id,
    );

    return asignaciones
      .filter((a: AsignacionSedeUsuario) => !!a?.get()?.sede?.get())
      .map((a: AsignacionSedeUsuario) => {
        const { asignacion, ...sede } = a?.get()?.sede?.get();
        return {
          ...sede,
          trabajadores: asignacion
